...
const SingleForumPost = ({ postDetails, setApiRecall }) => {
  ...
  const [allComment, setAllCommnet] = useState(
    JSON.parse(postDetails.commments)
  );
    ...
    allComment.map((comment) => {
      if (comment.commentId === commentId) {
        ...
        let prevRepliesWithNewReply = [
          ...comment.replies,
          { replierName: name, replyTime, reply: commentReply.current.value },
        ];
        newCommentsWithReply = [
          ...newCommentsWithReply,
          { ...comment, replies: { ...prevRepliesWithNewReply } },
        ];
      } else {
        newCommentsWithReply = [...newCommentsWithReply, comment];
      }
    });

    console.log(JSON.stringify(newCommentsWithReply));

    axios
      .post(`${rootAPI}/add_comment_to_forum_post`, {
        id: postDetails._id,
        comments: newCommentsWithReply,
      })
      .then((res) => {
        console.log(res);
      })
